...
export default function ProgressStepper({ currentStep }: ProgressStepperProps) {
  return (
    <div className="bg-white rounded-3xl shadow-2xl p-6 md:p-8 border border-gray-100 mb-8">
      <div className="relative">
        ...
        <motion.div 
          className="absolute top-8 left-0 h-1 bg-linear-to-r from-indigo-600 via-purple-600 to-pink-600 rounded-full"
          initial={{ width: '0%' }}
            ...
            return (
              <div key={step.id} className="flex flex-col items-center" style={{ width: '20%' }}>
                ...
                <motion.div
                  ...
                  className="w-12 h-12 md:w-16 md:h-16 rounded-full flex items-center justify-center mb-3 relative z-10 shadow-lg"
                >
                  {isCompleted ? (
                    <HiCheck className="w-6 h-6 md:w-8 md:h-8 text-white" />
                  ) : (
                    <Icon className={`w-6 h-6 md:w-8 md:h-8 ${isActive || isCompleted ? 'text-white' : 'text-gray-400'}`} />
                  )}
